...
const Appointment = () => {
 ...
 const getAvailableSlots = async () => {
   if (!docInfo) return; // <-- Add this guard
   ...
   for (let i = 0; i < 7; i++) {
     ...
     while (currentDate < endTime) {
       ...
       if (isSlotAvailable || docInfo.slots_booked?.[slotDate] === undefined) {
         timeSlots.push({
      ...
      <div>
        ...
        <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
              ...
              docSlots[slotIndex].map((item, index) => (
                <p
                  ...
                  className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${
                    item.time === slotTime
                      ? "background color: bg-blue-500 text-white"
          ...
          <button
            ...
            className="background color: bg-blue-500 text-white-sm font-light px-8 py-3 rounded-full my-6"
          >
            Book an Appoinment
          </button>
        </div>
        {/* List of the related doctors */}
